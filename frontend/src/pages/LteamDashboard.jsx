...
import { CheckCircle, XCircle, FileText, Download, Scale, ClipboardList } from "lucide-react";

export default function LTeamDashboard() {
  const [properties, setProperties] = useState([]);

  const team = localStorage.getItem("team") || "Legal Team";
  const userRole = localStorage.getItem("role") || "legal";
    ...
    axios
      ...
      .then((res) => {
        const filtered = res.data.filter((property) => {
          const isPendingForLegal = property.status === "Shortlisted";
          const rejectedByLegalTeam =
            property.rejected_by === "Legal Team" ||
            property.rejected_by === "Legal";

          return isPendingForLegal || rejectedByLegalTeam;
        });

        console.log("Filtered properties for Legal Team:", filtered.length);
      ...
      if (error.response?.status === 401) {
        ...
        alert(
          `Permission denied: ${
            ...
            "Please check your team permissions"
          }`
        );
      } else {
        alert(
  ...
  const rejectProperty = async (id) => {
    ...
    try {
      ...
      setProperties((prev) =>
        prev.map((p) =>
          p.id === id ? { ...p, status: "Rejected", rejected_by: userRole } : p
        )
      );
            ...
            <div className="mb-12">
              <div className="flex justify-between items-start md:items-center mb-8 gap-6 flex-wrap">
                <div>
                  ...
                  <p className="text-gray-600 text-lg">Review and approve legal compliance</p>
                </div>
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="bg-white rounded-xl p-6 shadow-md border-l-4 border-blue-600 flex-1">
                    <div className="flex items-center justify-between h-full">
                      ...
                      <Scale size={40} className="text-blue-600 opacity-20" />
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div>
              ...
              {properties.length > 0 ? (
                <div className="grid grid-cols-1 gap-4">
                  {properties.map((p) => {
                    const isRejectedByThisTeam =
                      p.rejected_by === team ||
                      (team === "Legal Team" &&
                        (p.rejected_by === "Legal" ||
                          p.rejected_by === "Legal Team"));
