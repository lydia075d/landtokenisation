...
import { CheckCircle, XCircle, FileText, Download, Cpu, ClipboardList } from "lucide-react";

export default function TechteamDashboard() {
  const [properties, setProperties] = useState([]);
   const team = localStorage.getItem("team") || "Tech Team";
  const userRole = localStorage.getItem("role") || "tech";
  ...
  useEffect(() => {
    ...
    .then(res => {
      const filtered = res.data.filter(property => {
         const isPendingForTech = property.status === 'Finance Approved' || 
                               property.status === 'Pending Tech Review';
         const rejectedByTechTeam = property.rejected_by === "Tech Team" ||
                                 property.rejected_by === "Tech";
        
        return isPendingForTech || rejectedByTechTeam;
      });
      
      console.log('Filtered properties for Tech Team:', filtered.length);
            ...
            <div className="mb-12">
              <div className="flex justify-between items-start md:items-center mb-8 gap-6 flex-wrap">
                <div>
                  ...
                  <p className="text-gray-600 text-lg">Verify technical documentation </p>
                </div>
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="bg-white rounded-xl p-6 shadow-md border-l-4 border-blue-600 flex-1">
                    <div className="flex items-center justify-between h-full">
                      ...
                      <Cpu size={40} className="text-blue-600 opacity-20" />
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                  {properties.map((p) => {
                    const isRejectedByThisTeam = p.rejected_by === team || 
                      (team === "Tech Team" && (p.rejected_by === "Tech" || p.rejected_by === "Tech Team"));
                ...
                <div className="bg-white rounded-xl p-12 shadow-md border-2 border-dashed border-gray-300 text-center">
                  <div className="mb-4 flex justify-center">
                    <div className="bg-red-100 rounded-full p-6">
                      <Cpu size={48} className="text-red-600" />
                    </div>
                  </div>
