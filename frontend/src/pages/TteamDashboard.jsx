...
import { CheckCircle, XCircle, FileText, Download, Coins, ClipboardList } from "lucide-react";

export default function TokenTeamDashboard() {
  const [properties, setProperties] = useState([]);
   const team = localStorage.getItem("team") || "Token Team";
  const userRole = localStorage.getItem("role") || "token";
  ...
  useEffect(() => {
    ...
    .then(res => {
      const filtered = res.data.filter(property => {
         const isPendingForToken = property.status === 'Tech Approved' || 
                                property.status === 'Pending Token Review';
         const rejectedByTokenTeam = property.rejected_by === "Token Team" ||
                                  property.rejected_by === "Token";
        
        return isPendingForToken || rejectedByTokenTeam;
      });
      
      console.log('Filtered properties for Token Team:', filtered.length);
            ...
            <div className="mb-12">
              <div className="flex justify-between items-start md:items-center mb-8 gap-6 flex-wrap">
                <div>
                  ...
                  <p className="text-gray-600 text-lg">Generate and validate property tokens</p>
                </div>
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="bg-white rounded-xl p-6 shadow-md border-l-4 border-blue-600 flex-1">
                    <div className="flex items-center justify-between h-full">
                      ...
                      <Coins size={40} className="text-blue-600 opacity-20" />
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                  {properties.map((p) => {
                    const isRejectedByThisTeam = p.rejected_by === team || 
                      (team === "Token Team" && (p.rejected_by === "Token" || p.rejected_by === "Token Team"));
