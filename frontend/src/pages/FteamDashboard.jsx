...
import { CheckCircle, XCircle, FileText, Download, DollarSign, ClipboardList } from "lucide-react";

export default function FteamDashboard() {
  const [properties, setProperties] = useState([]);
  const team = localStorage.getItem("team") || "Finance Team";
  ...
  useEffect(() => {
    ...
    .then(res => {
      const filtered = res.data.filter(property => {
        ...
        const isPendingForFinance = property.status === 'Shortlisted' || 
                                   property.status === 'Pending Finance Review';
         const rejectedByFinanceTeam = property.rejected_by === "Finance Team" ||
                                    property.rejected_by === "Finance";
        
        return isPendingForFinance || rejectedByFinanceTeam;
      });
      
      console.log('Filtered properties for Finance Team:', filtered.length);
            ...
            <div className="mb-12">
              <div className="flex justify-between items-start md:items-center mb-8 gap-6 flex-wrap">
                <div>
                  ...
                  <p className="text-gray-600 text-lg">Review financial viability and approve transactions</p>
                </div>
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="bg-white rounded-xl p-6 shadow-md border-l-4 border-blue-600 flex-1">
                    <div className="flex items-center justify-between h-full">
                      ...
                      <DollarSign size={40} className="text-blue-600 opacity-20" />
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 gap-4">
                  {properties.map((p) => {
                    const isRejectedByThisTeam = p.rejected_by === team || 
                      (team === "Finance Team" && (p.rejected_by === "Finance" || p.rejected_by === "Finance Team"));
