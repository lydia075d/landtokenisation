...
import { CheckCircle, XCircle, FileText, Download, Building2, ClipboardList } from "lucide-react";

export default function PTeamDashboard() {
  const [properties, setProperties] = useState([]);
  const team = localStorage.getItem("team") || "Property Team";
  const userRole = localStorage.getItem("role") || "property";
  ...
  useEffect(() => {
    ...
    axios
      .get(`http://localhost:5001/api/properties/all`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((res) => {
        const filtered = res.data.filter((property) => {
          const isPendingVerification = property.status === "Pending Verification";
          const rejectedByPropertyTeam =
            property.rejected_by === "Property Team" ||
            property.rejected_by === "Property";

          return isPendingVerification || rejectedByPropertyTeam;
        });

        console.log("Filtered properties for Property Team:", filtered.length);
  ...
  const approveProperty = async (id) => {
    ...
    const teamName = team;
    console.log("Approving property with:", { token: token ? "present" : "missing", team: teamName, propertyId: id });
    ...
    try {
      ...
      alert(`Property approved by ${teamName} and moved to next team!`);
    } catch (error) {
      ...
      if (error.response?.status === 401) {
        ...
        alert(`Permission denied: ${error.response?.data?.msg || "Please check your team permissions"}`);
      } else {
        alert(`Approval failed: ${error.response?.data?.msg || error.message}`);
      }
    }
  };
  ...
  const rejectProperty = async (id) => {
    ...
    try {
      ...
      alert(`Rejection failed: ${error.response?.data?.msg || error.message}`);
    }
  };
  ...
  const getApprovalProgress = (p) => {
    ...
    if (p.team_approvals) {
      try {
        approvals =
          typeof p.team_approvals === "string"
    ...
    for (let i = 0; i < defaultTeams.length; i++) {
      ...
      if (status === "Approved") visibleApprovals[currentTeam] = "Approved";
      else if (status === "Rejected" || p.rejected_by === currentTeam) {
        visibleApprovals[currentTeam] = "Rejected";
        break;
      } else {
        let allPrevApproved = defaultTeams.slice(0, i).every((t) => approvals[t] === "Approved");
        if (allPrevApproved) {
    ...
    return (
      <div className="flex flex-wrap gap-2 mt-3">
        {Object.entries(visibleApprovals).map(([teamName, status], i) => {
          ...
          return (
            <span key={i} className={`px-3 py-1 rounded-full text-xs font-semibold ${bg}`}>
              {teamName}: {status}
            </span>
          );
        })}
            ...
            <div className="mb-12">
              <div className="flex justify-between items-start md:items-center mb-8 gap-6 flex-wrap">
                <div>
                  ...
                  <p className="text-gray-600 text-lg">Approve or reject submitted properties</p>
                </div>
            ...
            <div>
              ...
              {properties.length > 0 ? (
                <div className="grid grid-cols-1 gap-4">
                  {properties.map((p) => {
                    const isRejected = p.rejected_by === team || p.rejected_by === "Property Team";
                      ...
                      >
                        <div className="p-6">
                          ...
                          <div className="flex flex-col sm:flex-row gap-3">
                            ...
                            <a
                              href={`data:application/json,${encodeURIComponent(JSON.stringify(p, null, 2))}`}
